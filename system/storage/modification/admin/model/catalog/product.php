...
//                                        debug($data['product_image']);
					foreach ($product_image['options'] as $povs) {
						if(isset($povs)){
							foreach($povs as $pov){
                                                //find product option id
//                                                debug($pov);
      ...
      if (!empty($data['gkd_extra_desc_fields']) && defined('GKD_UNIV_IMPORT')) {
        ...
        $gkd_prod_opt_val_id = defined('GKD_UNIV_IMPORT') && !empty($product_option['product_option_value_id']) ? "product_option_value_id = '" . (int) $product_option['product_option_value_id'] . "'," : '';
						...
						foreach ($product_option['product_option_value'] as $product_option_value) {
							...
							$this->db->query("INSERT INTO " . DB_PREFIX . "product_option_value SET ".$gkd_prod_opt_id." product_option_id = '" . (int)$product_option_id . "', product_id = '" . (int)$product_id . "', sku =  '" . $product_option_value['sku']. "', option_id = '" . (int)$product_option['option_id'] . "', option_value_id = '" . (int)$product_option_value['option_value_id'] . "', quantity = '" . (int)$product_option_value['quantity'] . "', subtract = '" . (int)$product_option_value['subtract'] . "', price = '" . (float)$product_option_value['price'] . "', price_prefix = '" . $this->db->escape($product_option_value['price_prefix']) . "', points = '" . (int)$product_option_value['points'] . "', points_prefix = '" . $this->db->escape($product_option_value['points_prefix']) . "', weight = '" . (float)$product_option_value['weight'] . "', weight_prefix = '" . $this->db->escape($product_option_value['weight_prefix']) . "'");
						}
					}
				} else {
