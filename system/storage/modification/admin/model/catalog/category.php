...
// Start products or gallery
      ...
      if (!empty($data['category_id']) && defined('GKD_UNIV_IMPORT')) {
        ...
        $univimp_item_id = '';
			...
			$this->db->query("INSERT INTO " . DB_PREFIX . "category SET " . $univimp_item_id . " parent_id = '" . (int)$data['parent_id'] . "', is_menu = '" . (int)$data['is_menu'] . "', `backend_only`='".(int)$data['backend_only']."', `top` = '" . (isset($data['top']) ? (int)$data['top'] : 0) . "', `column` = '" . (int)$data['column'] . "', category_names = '" . $this->db->escape($data['category_names']) . "', sort_order = '" . (int)$data['sort_order'] . "', status = '" . (int)$data['status'] . "', date_modified = NOW(), date_added = NOW()");
...
// Start products or gallery
		$this->db->query("UPDATE " . DB_PREFIX . "category SET parent_id = '" . (int)$data['parent_id'] . "', is_menu = '" . (int)$data['is_menu'] . "', `backend_only`='".(int)$data['backend_only']."', `top` = '" . (isset($data['top']) ? (int)$data['top'] : 0) . "', `column` = '" . (int)$data['column'] . "', category_names = '" . $this->db->escape($data['category_names']) . "', sort_order = '" . (int)$data['sort_order'] . "', status = '" . (int)$data['status'] . "', date_modified = NOW() WHERE category_id = '" . (int)$category_id . "'");
// End products or gallery
		if (isset($data['image'])) {
			$this->db->query("UPDATE " . DB_PREFIX . "category SET image = '" . $this->db->escape($data['image']) . "' WHERE category_id = '" . (int)$category_id . "'");
		}
