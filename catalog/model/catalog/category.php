...
class ModelCatalogCategory extends Model {
	...
	public function getCategories($parent_id = 0) {
		...
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "category c LEFT JOIN " . DB_PREFIX . "category_description cd ON (c.category_id = cd.category_id) LEFT JOIN " . DB_PREFIX . "category_to_store c2s ON (c.category_id = c2s.category_id) WHERE c.parent_id = '" . (int)$parent_id . "' AND cd.language_id = '" . (int)$this->config->get('config_language_id') . "' AND c2s.store_id = '" . (int)$this->config->get('config_store_id') . "'AND c.category_names='". $main_pcategories . "'  AND c.status = '1'  AND c.backend_only=0 ORDER BY LCASE(cd.name)");
        // End products or gallery
		return $query->rows;
	}
	
	
	public function getCategoriesLink($parent_id = 0) {
        // Start products or gallery
		if (isset($this->session->data['main_product']) && $this->session->data['main_product']) {
			$main_pcategories = $this->session->data['main_product']; 
		}else{
		...
		// debug($main_pcategories);
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "category c LEFT JOIN " . DB_PREFIX . "category_description cd ON (c.category_id = cd.category_id) LEFT JOIN " . DB_PREFIX . "category_to_store c2s ON (c.category_id = c2s.category_id) WHERE c.parent_id = '" . (int)$parent_id . "' AND cd.language_id = '" . (int)$this->config->get('config_language_id') . "' AND c2s.store_id = '" . (int)$this->config->get('config_store_id') . "'  AND c.status = '1' AND c.is_menu = 1  AND c.backend_only=0 ORDER BY LCASE(cd.name)");
        // End products or gallery
		return $query->rows;
	}
